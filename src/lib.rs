...
pub trait GameUserSettings {
    ...
    fn server_setting(&self, key: &str) -> Option<String> {
        self.server_settings_section()
            .and_then(|section| section.get(key).and_then(Clone::clone))
    }

    fn script_engine_game_session(&self, key: &str) -> Option<String> {
